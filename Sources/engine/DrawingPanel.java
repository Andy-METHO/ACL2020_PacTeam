	...
	public DrawingPanel(GamePainter painter) {
		...
		this.addComponentListener(new ComponentAdapter() {
			...
			public void componentResized(ComponentEvent e) {
				...
				float scaleRatio1 = getWidth()/(float)getPreferredSize().width;
				float scaleRatio2 = getHeight()/(float)getPreferredSize().height;

				float scaleRatio = Float.max(scaleRatio1, scaleRatio2);

				width = (int)(getPreferredSize().width * scaleRatio);
				height =(int)(getPreferredSize().height * scaleRatio);
